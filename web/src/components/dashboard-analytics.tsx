  ...
  return (
    <div className="min-w-0 rounded-lg border p-3">
      ...
      <p className="text-xs text-muted-foreground">
        Média: {formatNumber(average)} {unit}
      </p>
      {change === null ? (
        <p className="mt-1 text-xs text-muted-foreground">
          Sem média comparável
        </p>
      ) : (
        <p
          className={`mt-1 flex items-center gap-1 text-xs ${
            above ? "text-amber-400" : "text-emerald-400"
          }`}
        >
          {above ? (
...
export function AnalyticsComparison({
  ...
  return (
    <section className="mt-6 grid min-w-0 gap-4 lg:grid-cols-2" aria-label="Análises do dia">
      <Card>
        <CardHeader>
          <CardTitle>Distribuição calórica</CardTitle>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Calorias por refeição</CardTitle>
        </CardHeader>
        <CardContent className="min-w-0">
          <MealCaloriesChart data={mealData} />
        </CardContent>
      </Card>
      <Card>
        <CardHeader>
          <CardTitle>Hoje x média de 7 dias</CardTitle>
        </CardHeader>
        <CardContent className="grid min-w-0 gap-3 sm:grid-cols-2">
          <Comparison
            label="Calorias"
            current={summary.consumed.kcal}
            average={week.averages.kcal}
            unit="kcal"
          />
          <Comparison
            label="Proteína"
            current={summary.consumed.protein_g}
            average={week.averages.protein_g}
            unit="g"
          />
        </CardContent>
      </Card>
      <div className="grid min-w-0 gap-4 sm:grid-cols-2">
          ...
          <CardContent>
            {summary.goal === null ? (
              <p className="text-sm text-muted-foreground">
                Defina uma meta para estimar o restante do dia.
              </p>
            ) : (summary.remaining?.kcal ?? 0) <= 0 ? (
              <p className="text-sm text-muted-foreground">
                Meta de calorias já atingida.
              </p>
            ) : missingMeals.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                Todas as refeições já têm registro.
              </p>
            ) : (
              <div className="space-y-1 text-sm">
                <p>
                  <strong>{formatNumber(summary.remaining?.kcal ?? 0)} kcal</strong>{" "}
                  restantes
                </p>
                <p className="text-muted-foreground">
                  ≈ {formatNumber(estimatedPerMeal ?? 0)} kcal por refeição:{" "}
                  {missingMeals.join(", ")}.
                </p>
              </div>
            )}
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-sm text-muted-foreground">
              {recordedDays} de 30 dias com registro.
            </p>
          </CardContent>
        </Card>
      </div>
